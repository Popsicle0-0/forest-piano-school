...
>
> **最后更新**: 2026-08-29 · 当前正式版本 **v21.0**（森林制琴工坊视觉基础、航线地图、L1物质舞台、SVG图标系统）· 安全部署脚本为 `2f1130a`；OSS 自动部署工具为 `eaaf6d2`。
>
> **视觉唯一规范**：`docs/VISUAL-ATELIER.md`。它定义材质、图标、动效、层级和触控边界；不要再向项目添加 emoji 导航、泛渐变卡片或会覆盖玩法的“3D”装饰。
>
> ## 先读这三份“当前真相源”
...
| 项 | 当前值 |
|---|---|
| main 最新部署工具提交 | `69c6097` — OSS 清理旧 hash 资源遇到瞬时 5xx 时不阻断已完成的正式上传 |
| 最近功能版本提交 | `46084b6` — v20 自适应舞台、L3 五声音阶先听后放、L4 先教学后挑战 |
| 正式站发布方式 | `scripts/deploy.sh` → OSS + Cloudflare 自定义域名 |
| 备用站发布方式 | 同一脚本强推 `gh-pages` |

- 版本号必须同步维护两处（双重防 iOS PWA 缓存）：
  - `src/main.js` 的 `APP_VERSION = 'v20.1'`
  - `index.html` 的 `<script src="/src/main.js?v=20.1">`
- 版本号渲染：HUD 左侧灰底标签显示当前版本。
- v19.1–v20.0 真机热修复已落实：L1 直入/对位、L2 点选、L3 五声音阶先听后放与竖屏完整目标、L4 先教学后挑战、短横屏布局、Waveform 隐藏等，详见 `FIXLOG-v19.0.md`。
